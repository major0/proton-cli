...
// encryptAESGCM encrypts plaintext with AES-GCM and returns base64.
// Mirrors the wire format: nonce || ciphertext || tag.
func encryptAESGCM(t *testing.T, plaintext, key, ad []byte) string {
	t.Helper()
...
func TestGenerate_MockServer(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var req lumo.ChatEndpointGenerationRequest
		...
		responseAD := []byte(lumo.ResponseAD(requestID))

		// Encrypt a known plaintext for the response.
		encContent := encryptAESGCM(t, []byte("Hello from Lumo"), aesKey, responseAD)
		...
		lines := []string{
			sseData(t, lumo.GenerationResponseMessage{Type: "queued"}),
			sseData(t, lumo.GenerationResponseMessage{Type: "ingesting"}),
			sseData(t, lumo.GenerationResponseMessage{
				Type:      "token_data",
				Target:    lumo.TargetMessage,
				Content:   encContent,
				Encrypted: true,
			}),
			sseData(t, lumo.GenerationResponseMessage{Type: "done"}),
		}
...
func TestGenerate_Rejected(t *testing.T) {
	...
	if !errors.Is(err, lumo.ErrRejected) {
...
func TestGenerate_Harmful(t *testing.T) {
	...
	if !errors.Is(err, lumo.ErrHarmful) {
...
func TestGenerate_Timeout(t *testing.T) {
	...
	if !errors.Is(err, lumo.ErrTimeout) {
		t.Fatalf("err = %v, want ErrTimeout", err)
	}
}

// sseData serializes a message to an SSE data: line.
func sseData(t *testing.T, msg lumo.GenerationResponseMessage) string {
	t.Helper()
	b, err := lumo.MarshalSSE(msg)
	if err != nil {
