...
func TestRemove_EmptyFolderNonRecursive(t *testing.T) {
	...
	c := &client.Client{}
	// This will panic or error at the session.Client call — we use recover
	// to verify we got past the non-empty check.
	func() {
		defer func() {
			if r := recover(); r == nil {
...
func TestRemove_FileLink(t *testing.T) {
	...
	c := &client.Client{}
	// File links skip the children check — will panic at session.Client.
	func() {
		defer func() {
			if r := recover(); r == nil {
				t.Fatal("expected panic from nil session, got none")
...
func TestRemove_RecursiveSkipsChildrenCheck(t *testing.T) {
	...
	c := &client.Client{}
	// Recursive=true skips children check, goes straight to session call.
	func() {
		defer func() {
			if r := recover(); r == nil {
